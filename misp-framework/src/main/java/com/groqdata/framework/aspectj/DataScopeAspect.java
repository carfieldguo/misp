...
public class DataScopeAspect
{
    ...
    public static void dataScopeFilter(JoinPoint joinPoint, SysUser user, String deptAlias, String userAlias, String permission)
    {
        ...
        user.getRoles().forEach(role -> {
            if (DATA_SCOPE_CUSTOM.equals(role.getDataScope()) && StringHelper.containsAny(role.getPermissions(), Convert.toStrArray(permission)))
            {
        ...
        for (SysRole role : user.getRoles())
        {
            ...
            if (!StringHelper.containsAny(role.getPermissions(), Convert.toStrArray(permission)))
            {
                continue;
            }
            ...
            else if (DATA_SCOPE_CUSTOM.equals(dataScope))
            {
                if (scopeCustomIds.size() > 1)
                {
                    ...
                    sqlString.append(StringHelper.format(" OR {}.dept_id IN ( SELECT dept_id FROM sys_role_dept WHERE role_id in ({}) ) ", deptAlias, String.join(",", scopeCustomIds)));
                }
                else
                {
                    sqlString.append(StringHelper.format(" OR {}.dept_id IN ( SELECT dept_id FROM sys_role_dept WHERE role_id = {} ) ", deptAlias, role.getRoleId()));
                }
            }
            ...
            {
                sqlString.append(StringHelper.format(" OR {}.dept_id IN ( SELECT dept_id FROM sys_dept WHERE dept_id = {} or find_in_set( {} , ancestors ) )", deptAlias, user.getDeptId(), user.getDeptId()));
            }
        ...
        if (StringHelper.isEmpty(conditions))
        {
            sqlString.append(StringHelper.format(" OR {}.dept_id = 0 ", deptAlias));
        }
        ...
        if (StringUtils.isNotBlank(sqlString.toString()))
        {
            ...
            if (StringHelper.isNotNull(params) && params instanceof BaseEntity)
            {
